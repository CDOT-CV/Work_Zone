...
def upload_logs(contents, path, bucket_name):
    try:
        ...
        print(
            f"Error uploading log path: {str(path)} to bucket: {bucket_name}")
        print(str(e))
        return False
...
def compare_features(prod_feature, experimental_feature):
    ...
    properties_to_compare = ["start_date", "end_date",
                             "work_zone_type", "reduced_speed_limit_kph"]
    for property in properties_to_compare:
        if prod_feature['properties'].get(property) != experimental_feature['properties'].get(property):
            diff[f'properties/{property}'] = {'prod': prod_feature['properties'].get(
                property), 'experimental': experimental_feature['properties'].get(property)}

    # compare core_details of features
    core_details_to_compare = ["description", "update_date"]
    for detail in core_details_to_compare:
        if prod_feature['properties']['core_details'].get(detail) != experimental_feature['properties']['core_details'].get(detail):
            diff[f'properties/core_details/{detail}'] = {'prod': prod_feature['properties']['core_details'].get(
                detail), 'experimental': experimental_feature['properties']['core_details'].get(detail)}

    # compare geometry
    if prod_feature['geometry'] != experimental_feature['geometry']:
        diff['geometry'] = {'prod': prod_feature['geometry'],
...
def compare_wzdx_experimental_prod(experimental: dict, prod: dict) -> dict:
    ...
    for feature in experimental.get('features', []):
        ...
        if not prod_dict.get(key):
            total_diff[key] = {
                'diff': 'Feature not found in prod', 'experimental': feature}
            continue

        diff = compare_features(prod_dict.get(key), feature)
        if diff:
            total_diff[key] = {'diff': diff, 'prod': prod_dict.get(
    ...
    for k, remaining_feature in prod_dict.items():
        total_diff[k] = {
            'diff': 'Feature not found in experimental', 'prod': remaining_feature}
    return total_diff


# https://us-central1-cdot-oim-cv-dev.cloudfunctions.net/get_wzdx_data
def get_log(entry):
    BUCKET_NAME = os.getenv('BUCKET_NAME', 'cdot-cv-wzdx-rest-data')

    WZDX_PROD_ENDPOINT = os.getenv(
        'WZDX_PROD_ENDPOINT', 'https://data.cotrip.org/api/v1/wzdx')
    WZDX_EXPERIMENTAL_ENDPOINT = os.getenv(
        'WZDX_EXPERIMENTAL_ENDPOINT', 'https://data.cotrip.org/api/v1/wzdxExperimental')
