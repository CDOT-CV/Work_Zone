...
def test_parse_xml():
    ...
    with open(inputfile, encoding="utf-8-sig") as f:
        xml_string = f.read()
        actual_icone_data = cwz_translator.parse_xml_to_dict(xml_string)
        assert (
            actual_icone_data == cwz_translator_data.test_parse_xml_expected_icone_data
        )
...
def test_parse_polyline_from_linestring_valid_data():
    ...
    test_coordinates = cwz_translator.parse_polyline_from_linestring(test_polyline)
...
def test_parse_direction_from_street_name_southbound():
    ...
    output_direction = cwz_translator.parse_direction_from_street_name(test_road_name)
