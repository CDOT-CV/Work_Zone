...
_datastore_client = datastore.Client(project=f"cdot-rtdh-prod")  # settings.PROJECT
...
def get_wzdx():
    ...
    query = _datastore_client.query(namespace="rtdh_cache_latest", kind="RTDHCache")
...
def write_raw_pubsub(messages, topic):
    open("wzdx_experimental.json", "w").write(json.dumps(messages, default=str))
    return
    ...
    for msg in messages:
        _publisher_client.publish(topic_path, json.dumps(msg, default=str).encode())

    l.info(f"Published all {str(len(messages))} messages to PubSub topic {topic}")
...
def get_experimental_icone(wzdx):
    icone_msgs = [json.loads(open("./tests/data/icone_standard.json").read())]
...
def get_experimental_navjoy(wzdx):
    ...
    open("wzdx_navjoy.json", "w").write(json.dumps(navjoy_wzdx_msgs, default=str))
...
def get_experimental_atma(wzdx):
    ...
    if geotab_msgs:
        ...
        combined_events = attenuator.get_combined_events(geotab_msgs, wzdx)
...
def cron():
    ...
    l.info(f"Environment Variable: EXPERIMENTAL_TOPIC = {settings.EXPERIMENTAL_TOPIC}")
    l.info(
    ...
    open("wzdx_experimental_icone.json", "w").write(json.dumps(wzdx_exp_icone))
    wzdx_experimental = combine_experimental_feed(wzdx_experimental, wzdx_exp_icone)
    print(len(wzdx_experimental))

    # Navjoy 568
    wzdx_exp_navjoy = get_experimental_navjoy(wzdx_experimental)
    open("wzdx_experimental_navjoy.json", "w").write(json.dumps(wzdx_exp_navjoy))
    wzdx_experimental = combine_experimental_feed(wzdx_experimental, wzdx_exp_navjoy)
    print(len(wzdx_experimental))

    # ATMA/GeoTab
    wzdx_exp_atma = get_experimental_atma(wzdx_experimental)
    open("wzdx_experimental_geotab.json", "w").write(json.dumps(wzdx_exp_atma))
    wzdx_experimental = combine_experimental_feed(wzdx_experimental, wzdx_exp_atma)
    print(len(wzdx_experimental))
