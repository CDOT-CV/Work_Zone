...
class NavigatorApi:
    def __init__(self, routes: dict, route_api: RouteApi):
        self.routes = routes
        self.route_api = route_api

    def extract_milepost_from_node(self, node_id: str) -> float:
        """Extract the milepost from a node ID"""
        return float(node_id.split("-")[1])

    def add_temporary_node(
        self, G: nx.Graph, route_id: str, milepost: float, temp_node_id: str
    ):
        # Identify edges on this route in the graph
        route_edges = [
            (u, v, d)
            for u, v, d in G.edges(data=True)
            if route_id in u and route_id in v
        ]
        ...
        closest_edge = min(
            route_edges,
            key=lambda edge: abs(self.extract_milepost_from_node(edge[0]) - milepost)
            + abs(self.extract_milepost_from_node(edge[1]) - milepost),
        )
        u, v, data = closest_edge

        # Calculate distances for splitting
        u_milepost = self.extract_milepost_from_node(u)
        v_milepost = self.extract_milepost_from_node(v)
    ...
    def find_route_errors(self, route_id: str, milepost: float) -> bool:
        # check if the route and milepost are valid
        if route_id not in self.routes:
            return f"Invalid route id: {route_id}"
        if self.routes[route_id]["MMin"] > milepost > self.routes[route_id]["MMax"]:
            return f"Invalid milepost: {milepost} is not between {self.routes[route_id]['MMin']} and {self.routes[route_id]['MMax']}"
        return None
    ...
    def get_navigation_route(
        ...
        start_errors = self.find_route_errors(start_rid, start_mm)
        end_errors = self.find_route_errors(end_rid, end_mm)
        if start_errors or end_errors:
            if start_errors and end_errors:
                return f"Start route error: {start_errors}, and end route error: {end_errors}"
            elif start_errors:
                return f"Start route error: {start_errors}"
            else:
                return f"End route error: {end_errors}"
        ...
        if not self.add_temporary_node(G, start_rid, start_mm, temp_node_1):
            return []
        if not self.add_temporary_node(G, end_rid, end_mm, temp_node_2):
            return []
    ...
    def get_geometry(
        self, nav_routes: List[Tuple[str, float, float]]
    ) -> List[List[float]]:
        # each route entry has a route id and a start and end mile marker
        geometry = []
        for nav_route in nav_routes:
            route_id, start_mm, end_mm = nav_route
            geometry.extend(
                self.route_api.get_route_between_measures(route_id, start_mm, end_mm)
            )
        return geometry

    def process_route(self, route: List[str]) -> List[Tuple[str, float, float]]:
